...
+ Originally, the app was built on 1 html page, the app.component.html. Though the app was functional, the html file was large and could be made simpler through splitting our 1 page into multiple components.
...
+ You can assign an alias to the property by putting the name you want to use inside the () of the @Input decorator.
  ...
  *//----------------------------------------------------------------// ALIAS FOR CUSTOM EVENTS/PROPERTIES //---------------------------------------------------------------//*

   + When you do not want to use the same property outside of the component as you use inside of it, assign an alias to the custom event/property.

   + Assigning an alias for Custom Properties:
    - Pass an argument to @Input where the argument is the name you want to refer to outside of the component.

   + Assigning an alias for Custom Events:
    - Pass an argument to @Output where the argument is the name you want to refer to outside of the component.
...
*//----------------------------------------------------------------// LOCAL REFERENCES //---------------------------------------------------------------//*
  ...
  + Local references can be added to any HTML element by adding #[name of reference] to the element.
    ...
    - You can only use local references inside the html template, not in the typescript.
  ...
  *//----------------------------------------------------------------// @ViewChild //---------------------------------------------------------------//*
  ...
  + Instead of: @ViewChild('serverContentInput') serverContentInput: ElementRef; Use: @ViewChild('serverContentInput', {static: true}) serverContentInput: ElementRef;
  ...
  + Using @Viewchild:
    - Add a local ref to the element you want access to.
    - Under export class, add @ViewChild('[name of local ref]')
      * You could also pass the component type (name) instead of the string

  *//----------------------------------------------------------------// NGCONTENT //---------------------------------------------------------------//*

  + By default, everyting that is placed in the opening/closing tags of your custom components will be lost (removed from the DOM)

  + ng-content is a directive that passes information into a component from the outside.

  + Using ng-content:
    - In the place where you want to render the content, add <ng-content></ng-content>
      * This serves as a hook in the component to mark the place for Angular where it should add any content it finds between the opening/closing tags 
  ...
  *//----------------------------------------------------------------// @CONTENTCHILD //---------------------------------------------------------------//*
  ...
  + @ContentChild is simlar to @ViewChild, but it is used to get access to content which is stored in another component but then passed on via ng-content.    